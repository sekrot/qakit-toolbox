...
Chrome-расширение со швейцарским ножом утилит для тестировщиков и разработчиков. Side panel содержит 13 утилит (11 MVP + Password Generator + JSON to YAML), всё работает оффлайн, без обращений к серверу. Хранение настроек и истории — в `chrome.storage.local`. На первом этапе монетизации нет, но архитектура готова к freemium-разделению (feature flags + плейсхолдер `isPro()`).
